...
> **Version-skew limitation (Talos v1.13 resource-format change).** Talos v1.13 changed the protobuf format of five control-plane config resources — `EtcdConfig`, `KubeletConfig`, `ControllerManagerConfig`, `SchedulerConfig`, `APIServerConfig` — from `map<string,string>` to a structured message (a documented BREAKING change in the [Talos v1.13.0 release notes](https://github.com/siderolabs/talos/releases/tag/v1.13.0)). Because the server is compiled against a single machinery SDK version and `string`/`message` are not wire-compatible, the changed extra-arguments map does not round-trip across that boundary — **cross-version reads of these specific resources are unreliable; treat them as untrusted.** A prior cross-version read returned those `extraArgs` empty, but the exact failure mode (empty values, a decode error, or a mis-mapped field) was not pinned down, and the reverse pairing (an older build against a v1.13+ node) is affected for the same reason. Note that the startup range-warning above does **not** cover this skew — a v1.12 or v1.13 node is in-range either way, so it never fires for the affected boundary; runtime detection is tracked in issue [#210](https://github.com/Nosmoht/talos-mcp-server/issues/210). Talos publishes no client/server version-skew policy and recommends matching versions — **match the talos-mcp release to your Talos version** (pin an older talos-mcp release built against the matching machinery SDK), the same way you would match `talosctl`. All other resources and all 19 gRPC methods remain backward-compatible across the v1.9–v1.13 range.
