...
| `TALOS_MCP_ALLOWED_PATHS` | (all) | Comma-separated path prefixes permitted for `talos_read_file` and `talos_list_files` (e.g. `/etc,/proc`). Defense-in-depth — does not resolve remote symlinks. Unset or empty allows all paths. |
