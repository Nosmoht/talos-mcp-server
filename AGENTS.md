...
The blocks below are regenerated by the `refresh-tool-inventory` skill from the registration sites in `cmd/talos-mcp/main.go`, `internal/prompts`, and `internal/resources`. Do not hand-edit between the sentinel markers — changes are overwritten.
